...
XUla_stream_static xula_dev;

//Initialise the configuration parameters of the IP
XUla_stream_static_Config xula_config = {
	0,
	XPAR_ULA_STREAM_STATIC_0_S_AXI_CONTROL_BUS_BASEADDR
};
...
int XUlaSetup()
{
	return XUla_stream_static_CfgInitialize(&xula_dev, &xula_config);
}

//Start the IP before it computes over the input data
void XUlaStart(void *InstancePtr)
{
	XUla_stream_static *pExample = (XUla_stream_static *)InstancePtr;
	XUla_stream_static_Start(pExample);
}
