...
The input is a LxK matrix and Lx181 sensing matrix.
where L is the no. of antennas.
...
/*Function declaration for top function --> MUSIC(ULA)*/
...
#pragma HLS PIPELINE
	...
	QRF_mainLoop:for (int i = 0 ; i < 2 ; i++)
	{
		...
		hls::qrf<TRANSPOSED_Q, DIMRa, DIMRa, HLS_COMPLEX, HLS_COMPLEX>(A_in, q_out, r_out);
		...
		hls::matrix_multiply<hls::NoTranspose, hls::NoTranspose, DIMRa, DIMRa, DIMRa, DIMRa, DIMRa, DIMRa, DATA_TYPE, DATA_TYPE>(r_out, q_out, A_in);
...
#pragma HLS PIPELINE
	...
	/*The eigenvectors corresponding to the lowest (L-M)
...
void column_extract(DATA_TYPE mat[L][181], int ind, DATA_TYPE col[1][L])
{
#pragma HLS INLINE
...
template <typename DATA_TYPE, typename T,int DIMRa, int DIMC_VEC>
void MSG(DATA_TYPE A[L][181], DATA_TYPE V[DIMRa][DIMC_VEC],int doa_out[M])
{

#pragma HLS INLINE

	DATA_TYPE col_herm [1][L];  //hermatian of col
	...
	#pragma HLS PIPELINE II=4
		...
		/*If the value is peak, the get_doa function sorts it in peaks_max array*/
		if((i>1) & (is_peak<T>(P[i-1],P[i-2],P[i])))
		{
