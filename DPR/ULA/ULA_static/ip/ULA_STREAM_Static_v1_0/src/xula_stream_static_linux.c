...
typedef struct {
    ...
    XUla_stream_static_uio_map maps[ MAX_UIO_MAPS ];
} XUla_stream_static_uio_info;

/***************** Variable Definitions **************************************/
static XUla_stream_static_uio_info uio_info;
...
static int uio_info_read_name(XUla_stream_static_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XUla_stream_static_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XUla_stream_static_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XUla_stream_static_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XUla_stream_static_Initialize(XUla_stream_static *InstancePtr, const char* InstanceName) {
	XUla_stream_static_uio_info *InfoPtr = &uio_info;
