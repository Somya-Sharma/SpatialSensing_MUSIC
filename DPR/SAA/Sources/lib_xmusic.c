...
XSaa_static xmmult_dev;

XSaa_static_Config xmmult_config = {
	0,
...
int XMmultSetup(){
	return XSaa_static_CfgInitialize(&xmmult_dev,&xmmult_config);
}

void XMmultStart(void *InstancePtr){
	XSaa_static *pExample = (XSaa_static *)InstancePtr;
	XSaa_static_InterruptEnable(pExample,1);
	XSaa_static_InterruptGlobalEnable(pExample);
...
void XMmultIsr(void *InstancePtr){
	...
	XSaa_static_InterruptGlobalDisable(pExample);
	//Disable the local interrupt
	XSaa_static_InterruptDisable(pExample,0xffffffff);

	// clear the local interrupt
	XSaa_static_InterruptClear(pExample,1);
...
int XMmultSetupInterrupt()
{
	...
	result = XScuGic_Connect(&ScuGic, XPAR_FABRIC_SAA_STATIC_0_INTERRUPT_INTR, (Xil_InterruptHandler)XMmultIsr, &xmmult_dev);
	if(result != XST_SUCCESS){
		return result;
	}
	//print("Enable the Adder ISR\n\r");
	XScuGic_Enable(&ScuGic,XPAR_FABRIC_SAA_STATIC_0_INTERRUPT_INTR);
	return XST_SUCCESS;
}
...
//	//transfer indices to the Vivado HLS block
...
//	/* Wait for transfer to be done */
//	while (XAxiDma_Busy(&AxiDma, XAXIDMA_DMA_TO_DEVICE)) ;
////	xil_printf("transfer indices done \r\n");
